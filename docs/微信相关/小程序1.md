...
* 子组件使用properties接受父组件的传参,无法接受异步参数,因子子组件的还未等到父组件获得值，组件就已经加载了
* 可以使用observer模式，监听这个对象对他赋值
* 不推荐在observer中改变properties值，会造成无限循环
* 推荐使用组件内的data数据作为展示，监听patient的数据变化，给data赋值
...
* 弹窗组件使用(Toast组件也是类似的)，需要自定义组件，类似与在弹窗中添加图片内容，则只能使用组件调用形式
...
* van-group-cell组合,比较使用于页面展示信息，用户界面展示跳转，或者选择样式,van-group-cell包裹的van-cell默认会有border，
...
<text>年龄</text>
 ...
 <picker mode="date" value="{{date}}"start="1930-01-01" end="2020-7-31" bindchange="bindDateChange">
    <view class="picker">
    <text wx:if="{{age.length===0}}" class="empty-age">年龄 </text> <text class="age">{{age}}</text>
    </view>
  </picker>
...
van-cell的里面的picker选择器必须要有一个值（默认值）才可以触发，如果没有值是无法触发微信原生选择器

van-cell可以用来做表单，表单的必填字段的*可以用required表示
...
使用微信小程序的scroll-view将需要滚动的样式组件包裹

* scroll-view的属性值，在滚动页面使用弹窗时，会产生样式穿透，弹窗出现后页面依然可以滚动
* 使用scroll-view样式去控制滚动，当页面弹窗出现时，设置scroll-x或scroll-y禁止滚动即可

* 监听事件catch:touchmove="stopMove" stopMove return false
...
* z-index和组件层级，当使用vant的dialog组件，实现弹窗功能时，如果页面有scroll-view组件和icon组件，则弹窗出现时，icon组件依然可以被点击，着是由于组件所处的层级不同，所以dialog的遮罩层无法覆盖icon组件
